...
class Node:
    def __init__(self, horizpos, vertipos):
        ...
        self.h = 0
        self.neighbors = []
        self.wallState = False
        self.parent = None
    
    def show(self, screen, color):
        if self.wallState == True:
            color = (0,0,0)
        pygame.draw.rect(screen, color, (self.x*w, self.y*h, w-1, h-1)) # -1's so the rects dont overlap?
...
#-----------------------------------------------------------------------------------------------------------#
def changeNodeState(pos, newState): #TODO MAJOR make this a change node method
    x = pos[0] // w
    y = pos[1] // h
    if x <= 29:
        grid[x][y].wallState = newState
...
pygame.draw.rect(screen, (255, 255, 0), (602, 0, 97, 38))
smallfont = pygame.font.SysFont('Corbel', 20)
text = smallfont.render('start', True, (0, 220, 0))
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if pygame.mouse.get_pressed()[0]:
                changeNodeState(pygame.mouse.get_pos(), True)
            if pygame.mouse.get_pressed()[2]:
                changeNodeState(pygame.mouse.get_pos(), False)
            #if pygame.mouse.get_pressed()[0] and 601 <= mouse[0] <= 699 and 1 <= mouse[1] <= 37:
                #changeNodeState(pygame.mouse.get_pos(), True, 1)
        ...
        if len(openList) > 0:
            ...
            if complete == False:
                ...
                for neighbor in currentNode.neighbors:
                    if neighbor in closedList or neighbor.wallState:
